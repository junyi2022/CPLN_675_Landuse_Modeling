...
```{r}
reg <- lm(data=training, ozoneHigh ~ HwyDensity + distWater + sumDevelop + sumForest + distI95 + Population +
            distCities)
summary(reg)
...
```{r}
reg.lag <- lm(data=training, ozoneHigh ~ HwyDensity + distWater + sumDevelop + sumForest + distI95 + Population +
            distCities+ Ave_ozoneH)
...
*Can you calculate `training.summary` and create MAPE in the mutate statement?*
